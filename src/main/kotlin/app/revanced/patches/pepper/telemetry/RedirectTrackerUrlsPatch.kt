 ...
 *
 * URL inventory derived from THREE sources:
 *  1. patchinfo.txt v15 §3 — static decompilation of Pepper PL 8.13.00.
 *  2. Basic-flow mitm capture — a handful of trackers fire on first launch.
 *  3. Full-feature mitm capture — additional hosts not in patchinfo §3:
 *       - pubads.g.doubleclick.net
 *       - mediation.goog
 *       - admob-gmats.uc.r.appspot.com
 *       - obplaceholder.click.com
 *       - fundingchoicesmessages.google.com
 *       - beta4-dev.eu-central-1.dealsix-aws.pepper.com
 *
 * Hosts that leak in practice but are NOT in this map because their URLs
 * are runtime-built (no string constant in dex):
 *   us-event.app-install.bid                  Liftoff (Vungle backend) —
 *   cdn.liftoff-creatives.io                  killed by VungleProvider removal
 *   impression-east.liftoff.io                in BlockTelemetryComponents
 *   events.ads.vungle.com runtime paths       same.
 *   crashlyticsreports-pa.googleapis.com      Datatransport runtime URL —
 *   firebaselogging-pa.googleapis.com         accepted as residual leak (low
 *                                             volume; would crash app to
 *                                             disable per patchinfo §1 KEPT).
 */
@Suppress("unused")
val redirectTrackerUrlsPatch = bytecodePatch(
    ...
    val replacements = mapOf(
        ...
        "https://ocular.pepperdeals.com/"      to "https://127.0.0.1:1/",  // Pepper.com / global
        ...
        // Google Mobile Ads / AdMob — patchinfo §3 set
    ...
    execute {
        ...
        System.err.println(
            "[redirectTrackerUrlsPatch] rewrote $hitCount const-string sites + " +
                "$fieldHitCount static-field initial values across $classCount classes",
        )
    }
}
